...
% 3=无终点，距离约束（Dijkstra）
% 4=无终点，路段数约束（BFS）
% 5=有终点，最短距离（Dijkstra，无约束）
% 6=有终点，最少路段数（BFS，无约束）

constraintMode = 3; % 设置约束模式

% 偏好模式选择（1-7）
% 1=综合得分（原Total）
% 2=滨水路线（Water_Mtotal）
% 3=高绿化路线（NDVI_Mtotal + GVI_Mtotal）
% 4=视野开阔路线（SVI_Mtotal + Buildng_Mtotal）
% 5=夜间灯光充足路线（light_Mtotal）
% 6=设施便利路线（POI_Mtotal）
% 7=坡度平缓路线（slope_Mtotal）

preferenceMode = 1; % 设置偏好模式
%==============================================================================
...
%---------------------- 有终点模式（1/2）和最值路径模式（5/6）专用参数 ----------------------
if constraintMode == 1 || constraintMode == 2 || constraintMode == 5 || constraintMode == 6
    endNode = 1104;       % 终点（仅模式1/2需要）
    %（路线长度约束或路段数约束改到了运行时设置，容差默认，也可以改成用户自行设置）
    PathTolerance_1 = 250;
    PathSegTolerance_2 = 0;
%----------------------------------------------------------------------
...
% 模式合法性检测
if ~ismember(constraintMode, [1,2,3,4,5,6])
    error("constraintMode必须为1-6");
end
if ~ismember(preferenceMode, [1,2,3,4,5,6,7])
    error("preferenceMode必须为1-7");
end

% 模式提示信息
fprintf('当前约束模式：%d ', constraintMode);
...
fprintf('当前偏好模式：%d ', preferenceMode);
...
switch preferenceMode
    case 1
        Total = data.Total; % 综合得分（原逻辑）
    case 2
        Total = data.Water_Mtotal; % 滨水路线
    case 3
        Total = data.NDVI_Mtotal + data.GVI_Mtotal; % 绿化路线
    case 4
        Total = data.SVI_Mtotal + data.Buildng_Mtotal; % 视野开阔路线
    case 5
        Total = data.light_Mtotal; % 夜间灯光充足路线
    case 6
        Total = data.POI_Mtotal; % 设施便利路线
    case 7
        Total = data.slope_Mtotal; % 坡度平缓路线
    otherwise
        error("preferenceMode必须为1-7");
end

Totalreal = data.toatl_ori1; % 真实综合评价（仅用于输出，若需对应偏好模式的真实值可在此扩展）
% 构建节点列表与邻接矩阵
...
if constraintMode == 3 || constraintMode == 4
    fprintf('起点: 节点 %d（无终点）\n', startNode);
else
...
if constraintMode == 1
    ...
    totalDistances = startToAllDist + endToAllDist; % 每个节点的总距离
    totalDistances(totalDistances == Inf) = []; % 排除不可达节点
    if isempty(totalDistances)
        error('起点和终点之间无有效路径');
    end
    minTotalDist = min(totalDistances);
    maxTotalDist = max(totalDistances);
    
     % 核心优化1：向上取整到步长的整数倍（确保推荐值为整数且符合步长）
    distanceStep = 500; % 间隔（可调整）
    minRecommended = ceil(minTotalDist / distanceStep) * distanceStep;
    maxRecommended = floor(maxTotalDist / distanceStep) * distanceStep;
    
    % 生成推荐距离数组（确保数组有效）
    if minRecommended > maxRecommended
        recommendedDistances = minRecommended;
    else
        recommendedDistances = minRecommended:distanceStep:maxRecommended;
    end
    
    % 核心优化2：格式化输出（自动转换km/m单位）
    fprintf('\n检测到有效路径距离范围：%.0f米 - %.0f米\n', minTotalDist, maxTotalDist);
    fprintf('推荐基准距离：');
    for i = 1:length(recommendedDistances)
        d = recommendedDistances(i);
        if d >= 1000
            fprintf('%.1fkm', d/1000);
        else
            fprintf('%dm', d);
        end
        if i < length(recommendedDistances)
            fprintf('、');
        end
    end
    fprintf('\n');
    ...
    % 用户输入并验证
    fprintf('\n请输入基准距离（PathLength_1，需在%.0f-%.0f米之间）：', minTotalDist, maxTotalDist);
    PathLength_1 = input('');
    % fprintf('请输入容差（PathTolerance_1）：');
    % PathTolerance_1 = input('');
    minPathLength = PathLength_1 - PathTolerance_1;
    maxPathLength = PathLength_1 + PathTolerance_1;

    % 验证输入是否在有效范围
    if minPathLength < minTotalDist || maxPathLength > maxTotalDist
        error('输入的约束范围超出有效路径范围，请重新输入');
    else
        fprintf('基于路线长度约束筛选节点（%d-%d米）...\n', minPathLength, maxTotalDist);
    end
...
elseif constraintMode == 2
    ...
    totalSegments = startToAllPathWords + endToAllPathWords; % 每个节点的总路段数
    totalSegments(totalSegments == Inf) = []; % 排除不可达节点
    if isempty(totalSegments)
        error('起点和终点之间无有效路径');
    end
    minTotalSeg = min(totalSegments);
    maxTotalSeg = max(totalSegments);
    fprintf('\n检测到有效路径路段数范围：%d段 - %d段\n', minTotalSeg, maxTotalSeg);
    
    % 核心优化：向上取整到步长的整数倍
    segmentStep = 5; % 间隔（可调整）
    minRecommended = ceil(minTotalSeg / segmentStep) * segmentStep;
    maxRecommended = floor(maxTotalSeg / segmentStep) * segmentStep;
    
    % 生成推荐路段数数组（处理边界情况）
    if minRecommended > maxRecommended
        recommendedSegments = minRecommended;
    else
        recommendedSegments = minRecommended:segmentStep:maxRecommended;
    end
    
    % 优化输出格式（去除末尾逗号）
    fprintf('推荐基准路段数：');
    for i = 1:length(recommendedSegments)
        fprintf('%d段', recommendedSegments(i));
        if i < length(recommendedSegments)
            fprintf('、');
        end
    end
    fprintf('\n');
    ...
    % 用户输入并验证
    fprintf('\n请输入基准路段数（PathSegments_2，需在%d-%d段之间）：', minTotalSeg, maxTotalSeg);
    PathSegments_2 = input('');
    % fprintf('请输入容差（PathSegTolerance_2）：');
    % PathSegTolerance_2 = input('');
    minPathSegments = PathSegments_2 - PathSegTolerance_2;
    maxPathSegments = PathSegments_2 + PathSegTolerance_2;
    
    % 验证输入是否在有效范围
    if minPathSegments < minTotalSeg || maxPathSegments > maxTotalSeg
        error('输入的约束范围超出有效路径范围，请重新输入');
    else
...
elseif constraintMode == 3
    ...
    % 排除起点自身（startNode）和不可达节点（Inf）
    validIndices = (startToAllDist < Inf) & (1:numel(startToAllDist) ~= startNode);
    validDistances = startToAllDist(validIndices);
    
    if isempty(validDistances)
        error('除起点外无其他可达节点，请检查路网数据');
    end
    
    minTotalDist = min(validDistances);  % 此时为其他可达节点的最小距离
    maxValidDist = max(validDistances);  % 其他可达节点的最大距离
        
    % 生成推荐距离（向上取整到步长倍数，确保覆盖上下限）
    distanceStep = 500; % 间隔（可调整）
    minRecommended = ceil(minTotalDist / distanceStep) * distanceStep;
    maxRecommended = floor(maxValidDist / distanceStep) * distanceStep;
    
    % 处理边界情况（确保推荐值有效）
    if minRecommended > maxRecommended
        recommendedDistances = minRecommended; % 范围过小时强制使用最小合理值
    else
        recommendedDistances = minRecommended:distanceStep:maxRecommended;
    end
    
    % 格式化输出（转换为km/m）
    fprintf('\n检测到有效路径距离范围：%.0f米 - %.0f米\n', minTotalDist, maxValidDist);
    fprintf('推荐基准距离：');
    for i = 1:length(recommendedDistances)
        d = recommendedDistances(i);
        if d >= 1000
    ...
    % 用户输入并验证（同时检查上下限）
    fprintf('\n请输入基准距离（PathLength_3，需在%.0f-%.0f米之间）：', minTotalDist, maxValidDist);
    PathLength_3 = input('');
    % fprintf('请输入容差（PathTolerance_3）：');
    % PathTolerance_3 = input('');
    minPathLength = PathLength_3 - PathTolerance_3;
    maxPathLength = PathLength_3 + PathTolerance_3;
    
    % 验证输入是否在有效范围
    if maxPathLength > maxValidDist
        error('输入的约束范围超出有效路径范围（%.0f-%.0f米），请重新输入', minTotalDist, maxValidDist);
    else
    ...
    validNodeMask(startNode) = true; % 保留起点
    validNodes = find(validNodeMask);
    
elseif constraintMode == 4
    % 模式4：无终点，路段数约束（有上下限，最少路段数为1）
    startToAllPathWords = bfs_all_pathwords(adjMatreal, startNode);
    validSegments = startToAllPathWords(startToAllPathWords < Inf); % 排除不可达节点
    if isempty(validSegments)
        error('起点无可达节点，请检查路网数据');
    end
    maxValidSeg = max(validSegments); % 最大有效路段数（上限）
    minTotalSeg = 1; % 最少路段数固定为1（用户需求）
    
    % 生成推荐路段数（向上取整到步长倍数）
    segmentStep = 5; % 间隔（可调整）
    minRecommended = max(segmentStep, ceil(minTotalSeg / segmentStep) * segmentStep);
    maxRecommended = floor(maxValidSeg / segmentStep) * segmentStep;
    ...
    % 用户输入并验证（同时检查上下限）
    fprintf('\n请输入基准路段数（PathSegments_4，需在%d-%d段之间）：', minTotalSeg, maxValidSeg);
    PathSegments_4 = input('');
    % fprintf('请输入容差（PathSegTolerance_4）：');
    % PathSegTolerance_4 = input('');
    minPathSegments = PathSegments_4 - PathSegTolerance_4;
    maxPathSegments = PathSegments_4 + PathSegTolerance_4;
    
    % 验证输入是否在有效范围
    if maxPathSegments > maxValidSeg
        error('输入的约束范围超出有效路径范围（%d-%d段），请重新输入', minTotalSeg, maxValidSeg);
    else
        fprintf('基于路段数约束筛选节点（%d-%d段）...\n', minPathSegments, maxPathSegments);
    end
...
elseif constraintMode == 5
    % 模式5：直接计算起点到终点的最短距离（无约束）
    [shortestDist, shortestPath] = dijkstra(distMatreal, startNode, endNode);

    % 提取最短路径的指标
    validNodes = shortestPath; % 有效节点为最短路径上的节点
    fprintf('最短距离路径节点序列：');
    fprintf('%d ', shortestPath);
    fprintf('\n最短路径真实距离：%.2f米\n', shortestDist);

elseif constraintMode == 6
    % 模式6：直接计算起点到终点的最少路段数（无约束）
    % 用BFS计算最少路段数路径并生成路径序列
    [minSegments, shortestPath] = bfs(adjMatreal, startNode, endNode); % 替换dijkstra为bfs
    if isinf(minSegments) || isempty(shortestPath)
        error('起点到终点无可达路径');
    end
    validNodes = shortestPath; % 有效节点为最少路段路径上的节点
    fprintf('最少路段数路径节点序列：');
    fprintf('%d ', shortestPath);
    fprintf('\n最少路段数：%d段\n', minSegments);
end
...
% 仅在有终点的模式中处理endCoord（模式1/2/5/6）
if constraintMode ~= 3 && constraintMode ~= 4
    endCoord = uniqueNodes(endNode, :);
else
    endCoord = []; % 模式3/4无终点，无需定义endCoord
end

% 仅模式1-4更新为子路网，模式5-6保留完整路网
if ~ismember(constraintMode, [5,6])
    % 更新路网为有效节点子路网（仅模式1-4执行）
    ...
    distMat = distMat(validNodes, validNodes);       
    distMatreal = distMatreal(validNodes, validNodes);
    numNodes = size(uniqueNodes, 1);
end

% 重新定位起点（所有模式都需要）
startNode = find(ismember(uniqueNodes, startCoord, 'rows'));
% 仅在有终点的模式中重新定位终点（模式1/2/5/6）
if constraintMode ~= 3 && constraintMode ~= 4
    endNode = find(ismember(uniqueNodes, endCoord, 'rows'));
else
    endNode = []; % 模式3/4无终点，保持为空
end

% 模式有效性检查（区分有/无终点）
if constraintMode == 5 || constraintMode == 6
    if isempty(startNode) || isempty(endNode)
        error('起点或终点在路网中不存在，无法继续路径计算。');
    end
elseif constraintMode == 1 || constraintMode == 2
    if isempty(startNode) || isempty(endNode)
...
elseif constraintMode == 5
    ...
    [startDist_std, startDist_real, startSegments, startTotal_std, startTotal_real, startScore, ~] = ...
        dijkstra_enhanced_dual(distMat, distMatreal, totalMat, totalMatreal, scoreMat, adjMat, startNode);
    % 直接提取终点指标（最短路径）
    metricsMatrix = [endNode, startTotal_std(endNode), startDist_std(endNode), ...
        startSegments(endNode), startTotal_real(endNode), startDist_real(endNode), ...
        (startTotal_std(endNode)^w1)/(startDist_std(endNode)^w2), ...
        startScore(endNode), startTotal_std(endNode)/startDist_std(endNode), ...
        startTotal_std(endNode)/startSegments(endNode)];
    rowIndex = 2;
...
elseif constraintMode == 6
    ...
    metricsMatrix = [endNode, startTotal_std(endNode), startDist_std(endNode), ...
        startSegments(endNode), startTotal_real(endNode), startDist_real(endNode), ...
        (startTotal_std(endNode)^w1)/(startSegments(endNode)^w3), ...
        startScore(endNode), startTotal_std(endNode)/startDist_std(endNode), ...
        startTotal_std(endNode)/startSegments(endNode)];
    rowIndex = 2;
end
...
%% 筛选最优路径（含得分相关指标）
if constraintMode == 5 || constraintMode == 6
    % 模式5/6只有一条路径，直接指定最优路径为生成的路径
    bestRow = metricsMatrix(1, :); % 唯一路径即最优
else
...
elseif constraintMode == 2
    % 模式2（有终点，路段数约束）：使用BFS重构
    if bestNode == -1
        [~, bestPath] = bfs(adjMatreal, startNode, endNode);
    else
        [~, startToMidPath] = bfs(adjMatreal, startNode, bestNode);
        [~, midToEndPath] = bfs(adjMatreal, bestNode, endNode);
        bestPath = [startToMidPath, midToEndPath(2:end)];
    end
elseif constraintMode == 3
    % 模式3（无终点，距离约束）：继续使用Dijkstra
    [~, bestPath] = dijkstra(distMatreal, startNode, bestNode);
elseif constraintMode == 4
    % 模式4（无终点，路段数约束）：使用BFS重构
    [~, bestPath] = bfs(adjMatreal, startNode, bestNode);
elseif constraintMode == 5
    % 模式5（有终点，最短距离）：直接使用Dijkstra生成的路径
    [~, bestPath] = dijkstra(distMatreal, startNode, endNode);
elseif constraintMode == 6
    % 模式6（有终点，最少路段数）：直接使用BFS生成的路径
    [~, bestPath] = bfs(adjMatreal, startNode, endNode);
end
...
%% 新增：BFS两点路径重构函数（用于模式2/4的路径生成）
function [segments, path] = bfs(adjMat, startNode, endNode)
    % 基于BFS计算从startNode到endNode的最少路段数路径
    % 返回：路段数(segments)和路径序列(path)
    numNodes = size(adjMat, 1);
    prev = zeros(1, numNodes); % 记录前驱节点
    visited = false(1, numNodes);
    queue = startNode;
    visited(startNode) = true;
    prev(startNode) = 0; % 起点无前驱
    ...
    segments = length(path) - 1; % 路段数 = 节点数 - 1
end