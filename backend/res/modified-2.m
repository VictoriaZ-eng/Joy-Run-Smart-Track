...
% 改进：①加了模式 5 和模式 6，分别是最短路径和最少路段数。
% ②约束改为在代码运行时用户输入，在输入前计算好能到达的最少距离和最远距离，并推荐参数，用户可以根据参数输入想要的长度或者路段数。
% ③容差改为不受用户控制（路线长度容差是 250 米，因为推荐的路线是 500 米间隔；
% 路段数容差是 0，可以精确到具体段个数的路段，距离不为 0 是因为每条道路的距离都不一样）。
...
elseif constraintMode == 5
    fprintf('当前模式：5（有终点，最短距离）\n');
    fprintf('起点: %d, 终点: %d\n', startNode, endNode);
    fprintf('目标：计算起点到终点的最短距离路径\n');
elseif constraintMode == 6
    fprintf('当前模式：6（有终点，最少路段数）\n');
    fprintf('起点: %d, 终点: %d\n', startNode, endNode);
    fprintf('目标：计算起点到终点的最少路段数路径\n');
end
